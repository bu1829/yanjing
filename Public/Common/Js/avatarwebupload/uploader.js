(function( factory ) {
    if ( !window.jQuery ) {
    	layer.confirm('jQuery is required', {icon: 5}, function(index){
			layer.close(index);
		});
    }
	        ...
	        if ( isBase64Supported ) {
	            ...
	            $.ajax('../../server/preview.php', {
	                ...
	                if (response.result) {
	                    ...
	                    layer.confirm('预览出错', {icon: 6}, function(index){
	            			layer.close(index);
	            		});
	                }
	            });
...
var Uploader = (function() {
    ...
    var fileCount = 0;
    if ( !Uploader.support() ) {
        layer.confirm('Web Uploader 不支持您的浏览器！', {icon: 5}, function(index){
			layer.close(index);
		});
    ...
    return {
        init: function( selectCb ) {
            uploader = new Uploader({
                ...
                fileSingleSizeLimit: singlemaxsize,

                server: fileupload,
                accept: {
                    title: 'Images',
                    extensions: "gif,png,jpg,jpeg,png",
                    mimeTypes: "image/*"
                },
                swf: './Uploader.swf',
                fileNumLimit: 100,
                /*onError: function() {
                    ...
                    layer.confirm(arguments[0], {icon: 5}, function(index){
            			layer.close(index);
            		});
                }*/
            });
            ...
            uploader.onUploadComplete = function(file,ret) {
            ...
            uploader.on('fileQueued', function( _file ) {
                ...
                uploader.makeThumb( file, function( error, src ) {
                    ...
                    if ( error ) {
                        layer.confirm('不能预览', {icon: 3}, function(index){
                			layer.close(index);
                		});
                        return;
                    }
...
Uploader.init(function( src ,file) {
	
	
    Croper.setSource( src );
    
    //显示遮盖层
    $("body").append("<div class='wen-dropdown'></div>")
    //显示wen-modal 就是放置图片的
    $(".wen-modal").fadeIn();
